...
export interface ProviderCliGates { network:boolean; generationCredentialEnv:string; judgingCredentialEnv:string; generationBudget:number; judgingBudget:number }
...
export function providerDistillation(config:DistillationConfig,gates:ProviderCliGates,env:NodeJS.ProcessEnv=process.env,factory:ClientFactory=defaultFactory):{generator:DistillationProvider;judge:DistillationProvider}{
  ...
  const build=(entry:DistillationConfig["generator"],credentialEnv:string,budget:number):DistillationProvider=>{
    ...
    return new ReliableTeacherProvider({transport,budgets:{global:budget,stage:budget,provider:budget,currency:"USD"},catalog:{price(){return {inputPerMillion:0,outputPerMillion:0,currency:"USD"};}},maxRetries:2,concurrency:2});
  };
  return {generator:build(config.generator,gates.generationCredentialEnv,gates.generationBudget),judge:build(config.judge,gates.judgingCredentialEnv,gates.judgingBudget)};
}
