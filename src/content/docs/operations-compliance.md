...
Production model/GPU commands are gated and are not run by the default verification suite. All five production embedding recipes remain unavailable unless their pinned download, license, hardware, architecture, reload and evaluation evidence passes. No timing, memory, benchmark, or support claim should be inferred from the CPU fixture.
