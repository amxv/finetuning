...
The status matrix currently contains 13 recipes: the tiny CPU embedding fixture is `supported` only for offline testing, while the Qwen 9B chat pilot and all eleven production recipes are `experimental`. Experimental status does not authorize execution: missing license, GPU smoke, clean reload, or evaluation evidence still fails closed. BGE sparse/ColBERT/hybrid and GTE sparse are later experimental work.

## Chat recipe status

| Canonical recipe             | Track / architecture         | Status       | Blocking evidence                             |
| ---------------------------- | ---------------------------- | ------------ | --------------------------------------------- |
| `qwen3.6-27b`                | chat / dense                 | experimental | pinned license, GPU smoke, reload, evaluation |
| `qwen3.6-35b-a3b`            | chat / MoE                   | experimental | pinned license, GPU smoke, reload, evaluation |
| `nemotron-cascade-2-30b-a3b` | chat / hybrid MoE            | experimental | pinned license, GPU smoke, reload, evaluation |
| `nemotron-3-nano-30b-a3b`    | chat / Mamba-transformer MoE | experimental | pinned license, GPU smoke, reload, evaluation |
| `olmo-3.1-32b-instruct`      | chat / dense instruct        | experimental | pinned license, GPU smoke, reload, evaluation |
| `olmo-3.1-32b-think`         | chat / dense reasoning       | experimental | pinned license, GPU smoke, reload, evaluation |
