...
class Phase20(unittest.TestCase):
    def test_chat_embedding_and_resume(self):
        for task in ("chat", "embedding"):
            value = job(task)
            self.assertEqual(parse_execution_job(value)["task"], task)
            self.assertEqual(len(canonical_job_hash(value)), 64)
        self.assertEqual(classify_resume({"complete": True}), "full")
        self.assertEqual(classify_resume({"complete": False}), "weights_only")

    def test_fail_closed(self):
        value = job()
        value["unknown"] = True
        with self.assertRaisesRegex(ValueError, "UNKNOWN_FIELD"):
            parse_execution_job(value)
