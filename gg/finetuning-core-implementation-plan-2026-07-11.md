...
Scope basis: `tmp/gg/model-selection-research.md`, `tmp/gg/distillation-research.md`, `tmp/gg/codebase-cli-sdk-research.md`, `tmp/gg/embedding-models-training-distillation-research.md`, `tmp/gg/runpod-support-research.md`, plus the current non-Markdown source and configuration.
...
After the unified local chat+embedding alpha, complete a product-wide documentation overhaul and make RunPod Pods with independent network volumes the first supported remote execution backend. The RunPod integration is a provider-neutral job-protocol consumer built on a committed, reviewed snapshot of the current bearer-authenticated `https://rest.runpod.io/v1` OpenAPI contract. It fails closed when a capability or representation is unknown. Legacy GraphQL and `runpodctl` are troubleshooting references, never SDK foundations. Long-running training uses Pods; optional Serverless support is later and limited to bounded evaluation, inference, serving, and fleet scheduling.
...
- A coherent documentation system spanning concepts, tutorials, task recipes, complete CLI/SDK/API/configuration references, model/provider matrices, operations, security/compliance, migration, troubleshooting, and tested end-to-end chat and embedding journeys.
- A pinned-current RunPod REST control-plane adapter, safe single-GPU Pod execution with an independent network volume, durable/resumable chat LoRA and embedding bi-encoder smoke workflows, artifact retrieval, lifecycle cleanup, and fail-closed cost/capability/security controls.
...
- Qualified RunPod QLoRA and supported single-node multi-GPU/distributed modes, interruption/spot execution only after pinned-contract and live semantic verification, capacity fallback, fleet scheduling, and optional RunPod Serverless evaluation/inference/serving.
...
- Claiming that RunPod enforces a provider-side hard dollar cap, offers generic Pod remote-exec/log-stream REST, exposes stable spot fields/eviction semantics, or has a directly usable Secrets resource representation until each is verified against the committed OpenAPI and opt-in live contracts.
- Using legacy RunPod GraphQL, undocumented console endpoints, or shelling out to `runpodctl` as the SDK control plane; using Serverless as the MVP long-running training scheduler; automatically deleting shared network volumes.
...
Documentation is a tested product surface: a concise landing README routes users through progressive disclosure into concepts, tutorials, how-to recipes, references, operations, and release/migration material. Commands and examples execute from clean packed artifacts; generated CLI/schema/model/provider reference is checked for drift; links, snippets, terminology, readability, and accessibility are release-gated.

Remote execution consumes the same versioned job envelope and artifact protocol as local execution. A RunPod controller plans and reconciles Pods, independent network volumes, immutable images, attempts, deadlines, checkpoints, costs, and cleanup without making the trainer or canonical datasets provider-specific. Unknown API states and unverified mutations are compatibility errors. Durable event files plus documented SSH/Jupyter/connect paths replace any invented exec/log endpoint. User-side elapsed-time/cost watchdogs and an in-container deadline are explicitly best-effort safeguards, not a provider hard cap.
...
@amxv/finetuning/execution       provider-neutral job, executor, capability, cost contracts
@amxv/finetuning/execution/runpod pinned REST control plane, planning, reconciliation, storage/connect adapters
...
finetuning runpod auth check
finetuning runpod doctor
finetuning runpod gpu list
finetuning runpod volume list|ensure|delete
finetuning runpod plan|launch|status|connect|cancel|stop|terminate|cleanup|resume|fetch|orphans|cost
```
...
RunPod mutating commands support `--dry-run`; destructive commands require confirmation or `--yes`. `plan` is read-only and resolves current capability, GPU, locality, storage and price evidence with timestamps. `launch` is the canonical name (`submit` may be a documented alias). Provider-neutral aliases such as `training run --executor runpod` may follow without weakening the explicit RunPod lifecycle UX. JSON stdout remains machine-readable while diagnostics use stderr. Unknown control-plane states, absent required response fields, or an unverified mutation surface fail closed; read-only diagnostics remain available where safe.
...
Phases 0-7 are the chat MVP, Phase 8 completes its alpha release, and Phases 9-10 are chat/general post-MVP extensions. Phases 11-17 are the first-class embedding MVP and Phase 18 completes the unified local chat + embedding alpha. Phase 19 is the distinct product-wide documentation overhaul. Phases 20-22 deliver and harden first-class RunPod Pod execution; Phase 23 is optional later Serverless/fleet scope. Explicitly marked items remain later extensions. Preserve this exact order under the existing fast-supervisor: assign and complete one phase at a time, use model preset `low` for every implementation, review, and acceptance agent, trust reported passing checks per the supervisor contract, and rotate implementers only under its context rule. Phase 18 is no longer the final branch-ready gate: the supervisor must continue through every appended phase, including Phase 23's bounded optional-scope contracts and non-goals, and then pass the amended Global Acceptance Gate.
...
### Phase 19 — Documentation Information Architecture, Cleanup, and Release-Grade Revamp (Required post-alpha gate)

**Phase goal and definition of done:** Replace accumulated phase-by-phase prose with a coherent, navigable, task-oriented documentation product without deleting necessary historical/migration guidance. Done when a new user can start from a concise landing README and, through progressive disclosure, complete runnable offline and opt-in live chat and embedding journeys; operators and SDK integrators can find authoritative concepts, how-to and reference material; every command/snippet/link/schema/model claim is checked; and documentation quality is a blocking release gate rather than a best-effort deliverable. This phase complements and consolidates Phase 17 rather than renumbering or erasing it.

**Files/file areas to read before starting:** `README.md`; docs-site navigation/configuration and all user-facing docs source authorized by the implementation lead; package manifests and export maps; CLI command registration/help/snapshots; public TypeScript declarations/API reports; Python public modules/CLI help; generated JSON Schemas; recipe/model/provider locks and capability matrices; runnable examples, fixtures, migration tests, release/changelog templates, and Phase 17 documentation outputs.

**Exact implementation work:**

- Define an information architecture with a concise README landing page, installation and five-minute offline start, separate Chat and Embeddings tracks, shared Concepts, Tutorials, How-to Recipes, CLI Reference, TypeScript SDK/API Reference, Python Trainer/API Reference, Configuration Schema, Model/Recipe Matrices, Provider/Execution, Operations, Security/Compliance, Troubleshooting, Migration, FAQ, and Release Notes. Add breadcrumbs, cross-links, “next step” routes, and beginner-to-operator progressive disclosure; archive or redirect duplicate/stale pages.
- Publish end-to-end copy-paste workflows for chat data -> distillation -> Qwen3.5 pilot/selected chat LoRA -> evaluation -> resume -> export, and embedding data -> mining/distillation -> Qwen3-Embedding bi-encoder -> evaluation -> resume -> export. Provide CLI and SDK equivalents, exact small inputs, expected output trees, dry-run/JSON examples, deliberate failure/recovery drills, and explicit boundaries between deterministic offline fixtures and paid/network/GPU steps.
- Document canonical data, manifests/provenance, job/event/artifact contracts, templates versus codecs, embedding prompts/pooling/objectives, recipe/model selection, provider capabilities, configuration precedence and strict schema, environment-reference secrets, structured exit/error codes, budgets/cost attribution, hardware sizing/probe workflow, checkpoint types/resume compatibility, export/upload, and cleanup.
- Add provider setup and credential hygiene; security threat model and redaction; privacy/retention/residency; source/teacher/student licensing and terms review; compliance attestations; lineage deletion and artifact provenance/SBOM; budgeting and “estimate versus enforcement” language; failure recovery, OOM/network/provider/checkpoint troubleshooting; legacy CLI/import/schema/protocol migration; version compatibility; FAQ; support/deprecation policy; and release documentation/checklist.
- Generate authoritative CLI, schema, public API, model/recipe and provider-capability reference where feasible from source contracts. Hand-written narrative links to generated versioned anchors and states its applicable package/protocol versions. Add terminology/glossary ownership so “run/job/stage/attempt,” “chat/response distillation,” and embedding vector/score/ranking concepts are consistent.
- Make every command and code block intentionally classified as executable, illustrative, or pseudocode. Verify executable blocks from a clean packed NPM consumer and clean Python wheel environment with no undeclared repository state. Pin/download-gate network assets and never place real credentials or mutable model references in examples.
- Enforce navigation/link/anchor checks, JSON/YAML/schema validation, shell command smoke tests, TypeScript typecheck/run, Python example tests, output/hash assertions, secret scanning, spelling/terminology rules, accessible heading hierarchy/alt text/table semantics/color contrast, readable line length and plain-language review. Add generated-reference drift detection, help/API/schema snapshot diffs, and code-snippet synchronization.

**CLI/SDK UX and public contracts:** Documentation promises only stable exports and commands present in packed artifacts. Every command page states syntax, required capabilities, defaults, config keys, environment references, stdin/stdout and JSON behavior, exit codes, mutations, cost/network/data-loss effects, examples, and version introduced. SDK pages distinguish browser-safe core, Node operations, Python trainer, stable versus experimental exports, and protocol compatibility. Model matrices are generated/checked from recipe locks and include exact revision, license snapshot, architecture, task, prompt/template/pooling, dimensions/context, precision/quantization, minimum measured hardware, support status, smoke evidence, and limitations.

**Required focused checks and acceptance tests:** Clean offline README quickstart; both end-to-end offline workflows through artifact inspection; clean tarball and wheel examples; all CLI help/reference parity; all stable SDK imports and snippets; configuration examples validated against schemas; links/anchors; example expected trees/hashes; secret scan; terminology and migration fixtures; accessibility/readability audit; versioned release-note/changelog test; stale generated-reference test that must fail CI. A person unfamiliar with the repository performs a timed task-based acceptance review and records ambiguities as blockers.

**Offline fake/contract testing and opt-in live smoke boundaries:** Normal CI runs deterministic fake providers, fake trainer/executor, local schemas, tiny chat/embedding datasets, and no paid calls, model downloads, GPUs, or uploads. Opt-in jobs separately verify current OpenAI/Anthropic setup, pinned model download/GPU commands, and later RunPod examples with explicit credentials, budget/runtime caps, immutable revisions, cleanup, and recorded evidence. Live results never make offline docs build nondeterministic.

**Risks/fallbacks:** Generated reference can become unreadable, so pair it with curated task-led guides while keeping one authoritative contract source. Hardware/cost/provider/license facts drift; timestamp and derive them from locks/live planning, label estimates, and block unsupported claims. If a production example is too costly, retain a tiny executable analogue and publish the production command only as explicitly opt-in with evidence requirements. Preserve redirects and migration tables rather than silently breaking inbound links.

**Dependencies/concurrency:** Depends on Phase 18's frozen public surfaces and release artifacts. Within the phase, information architecture, reference generation, examples, and quality tooling may be developed concurrently only after terminology and ownership freeze; the supervisor still closes Phase 19 as one phase before assigning Phase 20.

**MVP versus later scope:** Required now: complete IA, landing README, chat+embedding workflows, concepts/tutorials/how-to/reference/config/model matrices, security/compliance/provenance/budget/hardware/recovery/troubleshooting/migration/FAQ, clean-environment execution, and drift/accessibility/release gates. Later: translations, interactive notebooks, videos, hosted playgrounds, and expanded ecosystem integrations; none may replace the required textual and executable documentation.

### Phase 20 — RunPod Protocol Feasibility and Pinned REST Control-Plane Foundation (RunPod MVP foundation)

**Phase goal and definition of done:** Establish a provider-neutral remote job protocol and a secure, contract-tested RunPod control plane before exposing production mutation. Done when TS and Python independently validate versioned chat/embedding job, event, result and artifact envelopes; a CPU runner proves checkpoint/resume; a reviewed OpenAPI snapshot from `https://rest.runpod.io/v1/openapi.json` maps required Pod/network-volume/billing capabilities; fake-server contracts cover every intended request/state; unknown versions/states/fields fail closed; and a manual opt-in feasibility probe records the real lifecycle without claiming general orchestration support.

**Files/file areas to read before starting:** `contracts/` training/event/artifact schemas and generated bindings; `src/training`, `src/orchestration`, `src/node`, config/secret/redaction/error modules; Python runner protocol/checkpoint/artifact code; recipe locks for selected chat and embedding models; package exports/CLI foundations; fake runner/provider contract fixtures; CI secret/live-test policy; container/image build areas; Phase 19 docs/reference machinery.

**Exact implementation work:**

- Define/extend a discriminated `finetuning.amxv.dev/job/v1` envelope with run/attempt IDs, `chat|embedding`, recipe and immutable model/tokenizer/image revisions, input URIs/hashes, resource expectations, precision/quantization, checkpoint/evaluation/export policy, hard runtime deadline, and provider-neutral executor extension. Outputs are append-only events, complete checkpoint manifests, result and hash/media-typed artifact manifests.
- Create `RunPodTransport`, `RunPodControlPlane`, `RunPodObjectStore`, `RunPodConnector`, runtime validators, typed capability discovery, redacted structured errors, timeouts/abort, and narrow `@amxv/finetuning/execution` plus `./execution/runpod` exports. Separate REST lifecycle, S3-compatible volume transfer where verified, and SSH/Jupyter connection guidance.
- Commit a reviewed and checksummed current OpenAPI snapshot and generated/hand-reviewed DTO boundary; never generate on install. Map only verified `rest.runpod.io/v1` Pod, network-volume and billing operations. Add a scheduled/manual semantic-diff updater that produces review artifacts without silently changing normal CI.
- Implement config using `RUNPOD_API_KEY` or named environment references only, strict unknown-key handling, mode-`0600` local run state, bearer/header/signed-URL/token redaction, harmless auth checking, and typed read-only discovery. Direct RunPod Secrets representation remains unavailable until the pinned OpenAPI and live contract establish it; use only verified environment/secret injection surfaces and never persist values.
- Specify UUIDv7 run IDs, canonical spec hashes, attempt IDs, atomic provider-ID state, supported ownership markers, reconcile-before-retry after ambiguous mutations, adoption only on matching hash/image, and compatibility handling for unknown enum/state or incomplete payloads. Legacy GraphQL, undocumented console APIs, and `runpodctl` subprocess parsing are prohibited; `runpodctl` remains a human escape hatch only.
- Prove immutable image + independent network-volume job-envelope consumption, durable events/checkpoint, Pod recreation/resume, and artifact hash verification in an explicitly opt-in feasibility exercise. Do not invent generic Pod exec or REST log streaming: bootstrap via verified container entrypoint/template/environment surfaces; expose documented SSH/Jupyter/connect guidance and durable event-file reading.

**CLI/SDK UX and public contracts:** Add read-only `runpod auth check`, `runpod doctor`, `runpod gpu list`, `runpod volume list`, and `runpod plan --dry-run --json` only as their capabilities become verified. `RunPodCapabilitiesV1`, resolved-plan and compatibility-error schemas include OpenAPI revision/evidence timestamp. Mutations remain experimental/disabled until Phase 21. JSON never contains credentials or signed URLs. Unknown/unverified API behavior returns a stable compatibility error and permits safe diagnostics only.

**Required focused checks and acceptance tests:** Cross-language schema fixtures; unknown major/recipe/state/enum rejection; canonical hash/idempotency fixtures; CPU chat and embedding jobs; complete-versus-partial checkpoint resume; protocol/event ordering; OpenAPI checksum/generated DTO diff; request/response runtime validation; 401/403 classification; timeout/abort/reconcile ambiguity; redaction fuzz/snapshots; strict config; ownership/adoption refusal; fake Pod/volume/billing lifecycle; no imports of GraphQL/runpodctl SDK foundations.

**Offline fake/contract testing and opt-in live smoke boundaries:** Default CI uses an adversarial fake REST server and committed synthetic fixtures: success, delayed visibility, timeout-after-create, missing field, unknown state, 401/403/429/5xx, locality/capacity failure and billing lag. Live tests require `RUNPOD_LIVE_TEST=1`, a dedicated low-credit account, explicit cost/runtime caps and ownership prefix, and manual dispatch; this phase permits only harmless discovery plus a tightly scoped Pod/volume/checkpoint feasibility proof with guaranteed orphan review. Live behavior never silently updates contracts.

**Risks/fallbacks:** REST maturity and drift may invalidate fields; refuse mutation and emit a semantic-diff/compatibility report. Generic remote exec/log streaming is not established; use entrypoint bootstrap, durable events, documented SSH/Jupyter/SCP/S3 paths. Direct Secrets representation is not established; restrict injection to verified surfaces, recommend short-lived least-privilege tokens, and otherwise block. If REST cannot express a required mutation, do not fall back to GraphQL or CLI parsing; keep the capability unavailable.

**Dependencies/concurrency:** Depends on Phases 6, 7, 15, 18, and 19 contracts/docs. Protocol, fake server, OpenAPI review, and CPU runner can proceed concurrently after envelope freeze; security review and feasibility evidence must converge before completion. Blocks Phase 21.

**MVP versus later scope:** MVP foundation includes protocol, pinned REST, Pod/volume/billing capability discovery, secure config/redaction, fake contracts, CPU runner, and feasibility evidence. It excludes supported Pod mutation, multi-GPU, spot, Serverless, fleet scheduling, provider hard caps, generic exec/log APIs, and unverified Secrets resources.

### Phase 21 — Safe Single-GPU RunPod Pod Lifecycle and Durable Chat + Embedding Smoke Workflows (RunPod MVP)

**Phase goal and definition of done:** Deliver a safe one-Pod-per-run, single-GPU execution path backed by an independently retained network volume. Done when a fresh-account user can init, doctor, plan, launch, inspect status, obtain connect guidance, cancel/stop/terminate safely, clean up, resume, and retrieve verified artifacts for tiny chat LoRA and embedding bi-encoder workflows; failures after every remote mutation reconcile without duplicates; deadlines stop compute; and no unexpected billable Pod remains after the acceptance run.

**Files/file areas to read before starting:** Phase 20 protocol/REST/capability/security/fake-server code and evidence; orchestration state/leases/resume; local trainer cancellation/events/checkpoints/artifacts; selected Qwen3.5 chat pilot and Qwen3-Embedding-0.6B recipes; container/bootstrap definitions; dataset/model/cache/secret handling; CLI parser/output/exit-code conventions; Phase 19 RunPod docs skeleton and clean-example harness.

**Exact implementation work:**

- Implement `runpod init|doctor|plan|launch|status|connect|cancel|stop|terminate|cleanup|resume|fetch|orphans|cost` plus `volume list|ensure|delete`. Plan resolves verified GPU/capacity, VRAM, datacenter/volume locality, immutable image digest, disk/cache/checkpoint headroom, current price evidence, estimated upper bound, exact mutations, and warnings. All reads support JSON; mutations support dry-run; destructive operations identify data/billing scope and require confirmation/`--yes`.
- Stage canonical immutable job/data bundles under `/workspace/runs/<run-id>/input` on an independent network volume; keep checkpoints/events/results/artifacts under the run prefix and shared cache under a separately quota/lock-managed prefix. Disposable container/Pod disk never becomes the only checkpoint location. Pin image/base/system/Node/Python/CUDA/PyTorch dependencies and record observed driver/runtime.
- Bootstrap through a verified container entrypoint/template/environment path. Inject only named environment references through verified surfaces, never secrets in job JSON/flags/state/logs. Handle model/data downloads with immutable hashes, offline/cache validation, least-privilege read tokens, and explicit separate upload credentials only for user-requested publishing.
- Add atomic idempotent lifecycle state and reconcile-before-retry for create/start/stop/delete ambiguity; attempt-scoped events; adoption/ownership checks; durable cursors; bounded retries/concurrency; signal-aware emergency checkpoint; complete-checkpoint-only resume compatibility; artifact hash verification; and ownership-filtered cleanup/orphan reporting. Termination never deletes a shared volume; run-prefix deletion and volume deletion are separately confirmed.
- Implement user-side budget and time safeguards: current price/capacity evidence at plan/launch, mandatory runtime deadline, estimated `maxUsd`, local elapsed/billing watchdog while the controller is alive, in-container deadline, idle detection, success-stop default, orphan scans, and explicit warning that stopped resources may continue storage billing. There is no verified provider-side hard dollar cap; never label `maxUsd` as one or promise zero overspend during control-plane failure.
- Provide `status` by reconciling provider state and durable result/event files. `connect` emits verified SSH/Jupyter/IDE guidance. Logs/events read durable supported paths or documented connection mechanisms; generic Pod exec/log-stream REST remains unavailable. Implement chat LoRA and embedding bi-encoder tiny smoke jobs with preflight, checkpoint/resume, evaluation, export, fetch and cleanup.

**CLI/SDK UX and public contracts:** `RunPodPlanV1`, `RunPodRunStateV1`, `RunPodCostEstimateV1`, lifecycle result, cleanup manifest and artifact-fetch reports are versioned. Stable exits distinguish usage/config, auth, capacity, budget estimate, remote transient, incompatible revision/API, and partial cleanup. `launch` prints the resolved resource IDs, ongoing billing scope, deadline, storage retention and recovery commands. `cancel` concerns the logical job; `stop` preserves Pod/volume state as documented; `terminate` deletes the Pod only by default; `cleanup` reports exactly what it did and retained.

**Required focused checks and acceptance tests:** Plan determinism with timestamped dynamic evidence; GPU/VRAM/locality/storage rejection; cache quota/disk checks; missing-secret and redaction tests; fake lifecycle success plus failure after each mutation; timeout-after-create reconciliation; duplicate launch/resume; status/event cursor; SIGTERM checkpoint; incompatible resume/fork; budget/deadline/idle triggers; billing-lag warnings; artifact hash/reload; foreign-resource cleanup refusal; shared-volume retention; JSON/stdout-stderr/exit snapshots; end-to-end tiny chat and embedding smoke including deliberate interruption.

**Offline fake/contract testing and opt-in live smoke boundaries:** CI runs the full lifecycle against the fake server, local object-store/connector doubles and CPU/tiny trainer; it simulates capacity loss, stale billing, controller crash, orphan, corrupt/partial checkpoint and remote error without network. Opt-in live smoke requires a dedicated account, explicit on-demand GPU, tiny immutable jobs, strict runtime/estimated spend ceilings, unique ownership, independent volume, final fetch/hash check, Pod deletion and orphan scan. It does not test spot, generic exec/log REST, direct Secrets resources, or automatic volume deletion.

**Risks/fallbacks:** Volume locality can eliminate requested capacity; planner presents explicit GPU/region/new-volume alternatives and requires selection, never silently relocates data. Control-plane loss can outlive local watchdogs; in-container deadline and orphan recovery reduce but cannot eliminate overspend. If S3-compatible access is absent, use verified SSH/SCP/cloud-sync/connect instructions. If secret injection is unverified, block the dependent workflow or use a verified short-lived environment injection path. Default to on-demand Pods.

**Dependencies/concurrency:** Depends on completed Phase 20 and the chat/embedding training/evaluation/artifact contracts. Controller, CLI, image/bootstrap, and recipes can be developed concurrently against frozen fake contracts, but lifecycle integration and live acceptance close sequentially. Blocks Phase 22.

**MVP versus later scope:** MVP is safe single-GPU, one on-demand Pod per run, independent volume, two tiny workflows, durable checkpoint/artifacts, lifecycle/cost/cleanup safeguards. QLoRA breadth, multi-GPU, capacity automation, spot/interruption qualification, publishing and fleet/serverless remain later.

### Phase 22 — RunPod Training Hardening, QLoRA, Qualified Distributed Modes, Recovery, and Cost Accounting (Required hardening)

**Phase goal and definition of done:** Harden RunPod execution for the selected supported chat and embedding architectures without overstating provider or framework capabilities. Done when QLoRA and each advertised single-node multi-GPU/distributed mode passes architecture-specific preflight, training, checkpoint/recovery, evaluation/export/upload and artifact-reload gates; capacity fallback is explicit; retries/concurrency are bounded; and estimated costs reconcile with available billing history. Spot remains disabled unless its exact contract and eviction semantics pass pinned/live qualification.

**Files/file areas to read before starting:** Phase 21 lifecycle/state/images/recipes/tests; all chat recipe locks/template/target-module/kernel gates; all embedding recipe locks/prompt/pooling/objective/native-head gates; Python Accelerate/torchrun/distributed, QLoRA/PEFT, checkpoint/export/evaluation code; RunPod capability/price/billing fixtures; artifact upload/provider registry; CI GPU matrices and release support-status machinery.

**Exact implementation work:**

- Add QLoRA profiles with exact quantization/kernel/precision compatibility, target-module discovery/coverage reports, adapter save/reload parity and memory probes. Apply each existing chat architecture gate (template/reasoning, dense/MoE/hybrid module coverage, kernel and license pins) and embedding gate (pooling/prompt/padding/normalization/dimension, MoE/router/native-head/remote-code policy) before allocation and again in-container.
- Qualify only declared single-node multi-GPU modes, initially Accelerate/`torchrun` DDP or supported sharding per recipe. Verify visible devices, topology, NCCL collectives, effective batch semantics, sampler determinism, checkpoint world-size assumptions, metrics equivalence/tolerance and resume. Multi-node is a separate future proposal; never infer support from GPU count.
- Harden complete checkpoint publication, periodic/time-based and emergency checkpoints, forced controller/Pod/process interruption, resume loss bounds, corrupt checkpoint fallback, attempt provenance and weights-only warm-start classification. Interruptible/spot is opt-in experimental only after the exact current create field, pricing, eviction signal and lifecycle are verified by the pinned OpenAPI and dedicated live contracts; otherwise expose no stable spot flag and use on-demand Pods.
- Add explicit ranked capacity alternatives across verified GPU types/datacenters compatible with volume locality and recipe requirements; require confirmation when cost/hardware changes. Bound discovery/mutation retries and concurrency, respect retry hints, reconcile ambiguous operations, and prevent capacity fallback from silently changing precision, quantization, model, GPU count or distributed mode.
- Reconcile plan estimates, elapsed watchdog data and provider billing-history reads where the contract supports them; report compute, storage and retained-resource costs separately with timestamps/uncertainty. Preserve the boundary that no provider-side hard dollar-cap primitive is verified. Add cache quotas/eviction, idle/orphan safeguards, stop/terminate policy and post-run billable-resource inventory.
- Integrate evaluation, best-checkpoint selection, portable export, optional explicit upload with separate write credential, model-card/provenance/SBOM, and local `fetch` hash verification. Add resource-aware concurrency for multiple independent runs without sharing mutable output prefixes or secret state.

**CLI/SDK UX and public contracts:** Extend plan/doctor with `distributedMode`, topology, fallback candidates, QLoRA/module evidence, checkpoint compatibility, cost uncertainty and recipe support status. `resume` reports recovery point/loss window and refuses incompatible world size unless an explicitly supported reshard path exists. `cost` separates estimate/observed/billed and never calls the estimate a cap. Upload is a separate explicit action/policy and never occurs by default. Spot-related fields remain absent/experimental until verified evidence is embedded in capability output.

**Required focused checks and acceptance tests:** Per-architecture preflight negatives and supported smoke matrix; QLoRA forward/backward and module coverage; adapter reload parity; NCCL preflight and multi-GPU equivalence; uninterrupted versus resumed metrics tolerance; forced interruption/emergency checkpoint; corrupt/latest checkpoint fallback; capacity alternative confirmation; retry/concurrency/idempotency stress; cache quota and disk exhaustion; estimate-versus-billing report fixtures; evaluation/export/upload/fetch hashes; no secret leakage; retained-resource/orphan scan; support status cannot exceed evidence.

**Offline fake/contract testing and opt-in live smoke boundaries:** Offline CI uses GPU/framework capability doubles, multi-process CPU tests where meaningful, fake capacity/billing and simulated evictions without claiming real spot support. Opt-in GPU jobs are manually dispatched with immutable images/models, on-demand instances, explicit ceilings, volume retention and cleanup. Dedicated spot tests may begin only after OpenAPI review establishes fields and a low-credit live protocol verifies eviction/signal/billing/resume semantics; failure keeps spot unavailable. Paid multi-GPU and large-model matrices are scheduled release evidence, never PR defaults.

**Risks/fallbacks:** Framework support for 2026 architectures is volatile; freeze tested image/model/framework combinations and mark recipes unavailable rather than silently omit modules. Multi-GPU may not improve small jobs; retain single-GPU fallback only when the recipe fits and the user confirms cost/time. Capacity fallback may conflict with volume locality; require a new volume/transfer decision. Billing history may lag; state uncertainty and retain elapsed estimates. On-demand Pods are the safe fallback for every unverified interruption semantic.

**Dependencies/concurrency:** Depends on Phase 21 and the architecture gates from Phases 7, 11, 15, and 16. QLoRA, distributed runtime, recovery, capacity/cost, and export work can proceed concurrently against fixed contracts; each recipe is advertised only after its combined gate closes. Blocks branch-ready acceptance and Phase 23 integration.

**MVP versus later scope:** Required hardening includes QLoRA, explicitly supported single-node distributed modes, checkpoint recovery, capacity fallback, accounting, evaluation/export/fetch, bounded concurrency/retries, and selected-model gates. Later/unavailable until verified: spot, multi-node, automatic cross-region migration, autonomous budget guarantees, and unsupported architecture/native heads.

### Phase 23 — Optional RunPod Serverless Evaluation/Inference and Fleet Scheduling Boundaries (Later, bounded scope)

**Phase goal and definition of done:** Add an optional, isolated adapter for bounded evaluation/inference/serving and, separately, qualified fleet scheduling without turning Serverless into the long-running training MVP. Done when endpoint jobs have explicit input/output/runtime limits, observable queue/job states, cancellation/purge and cleanup semantics verified against current contracts; serving and training images/lifecycles remain separate; cost/cold-start/scale-to-zero behavior is measured; and any Pod pool preserves tenant/run isolation and idempotency. If live qualification is not authorized, done means the contracts, capability gates, docs and fail-closed unavailable status are complete—never a fabricated feature.

**Files/file areas to read before starting:** Phases 20-22 execution abstractions and capability evidence; provider-neutral evaluation/inference APIs; RunPod Serverless queue/endpoint contract fixtures and current pinned documentation/OpenAPI evidence; serving/export artifacts; worker images; auth/redaction/cost/cleanup code; concurrency/dispatcher state; Phase 19 generated docs and release support-status gates.

**Exact implementation work:**

- Add a separate Serverless adapter for bounded `/run`/`runsync`, status, cancel and purge only where revalidated, with versioned request/result/capability schemas, maximum payload/runtime/output, queue-delay and job-state handling, retries/idempotency, endpoint ownership and cleanup. Never route checkpointed long-running training through it.
- Build separate immutable evaluation/inference/serving worker images. Validate model/runtime compatibility; do not assume vLLM supports every embedding architecture. Measure cold start, active/idle workers, scale-to-zero, queue delay, cancellation and cost using Serverless pricing rather than Pod assumptions.
- Optionally implement a durable dispatcher/Pod pool for teams only after one-Pod-per-run semantics remain the reference. Preserve isolated run prefixes, credentials, caches/quotas, attempts, cancellation, cost attribution and cleanup; prevent one run from adopting or deleting another's resources. Multi-node training remains out of scope.
- Assess Flash only as an optional Python application deployment convenience, not as the TypeScript control plane. Keep Public Endpoints as an explicit evaluation convenience, never a training backend.

**CLI/SDK UX and public contracts:** Add explicit `runpod serverless evaluate|infer|status|cancel|cleanup` and fleet commands only when capabilities are verified; otherwise `doctor` reports unavailable with evidence. Require bounded specs and surface queue, execution timeout, worker scaling, cold-start and billing semantics. Keep endpoint IDs/provider-native settings namespaced. Training CLI continues to recommend Pods and rejects Serverless for long-running/checkpointed training.

**Required focused checks and acceptance tests:** Fake queue state machine including timeout/cancel/purge/unknown state; payload/runtime/output limit enforcement; idempotency/retry; secret/redaction; endpoint cleanup; scale configuration validation; cost schema separation from Pods; serving artifact reload; embedding compatibility negative; pool isolation/fairness/orphan recovery; CLI rejection of long-running training on Serverless; docs/support-status parity.

**Offline fake/contract testing and opt-in live smoke boundaries:** Normal CI is fake-only. Live Serverless tests require a dedicated endpoint/account, explicit low cost/runtime, tiny bounded evaluation/inference, cleanup and recorded cold-start/scale/cancel evidence. Fleet tests use synthetic jobs before any paid pool. No live evidence means `unavailable`/experimental status, not support.

**Risks/fallbacks:** Serverless contracts/pricing/states can drift and model workers may be incompatible; fail closed and retain local/Pod evaluation. Queue cancellation may not stop already running work immediately; disclose measured semantics. Pools increase isolation and orphan risks; retain one Pod per run. Do not use GraphQL, undocumented console APIs or Flash to bypass missing control-plane capabilities.

**Dependencies/concurrency:** Depends on the stable execution boundary and completed Phase 22 safeguards. Serverless worker, queue contract and optional dispatcher can be researched concurrently but ship independently behind capabilities. This phase is ordered last and must be explicitly accepted—even if the accepted result is a documented, tested unavailable boundary—before branch-ready declaration.

**MVP versus later scope:** Serverless and fleet work are later and optional product capabilities. Required for this plan's closure is the bounded design, rejection of long-running training, fake contracts, documentation and honest capability status. Actual supported endpoints/pools require opt-in live evidence; multi-node training and autonomous fleets are separate proposals.

## Global Acceptance Gate

The initiative is branch-ready or ready for stable promotion only when: current public workflows remain compatible; chat and embedding canonical schemas and contracts have documented version rules; OpenAI/canonical/HF chat and ST/HF embedding codecs have round-trip/loss coverage; retries and resume cannot duplicate paid successful calls; compliance fails closed; locked held-out chat or embedding evaluation data never enters teacher generation, mining, or candidate pools; the Qwen3.6-27B primary chat path and Qwen3-Embedding-0.6B primary embedding path train and resume end to end; each other advertised model has passed its declared hardware smoke gate; embedding evaluation clears declared regression thresholds against pinned baselines; and clean packed NPM/Python artifacts install and run.

In addition, every appended Phase 19-23 definition of done must be closed in order. Documentation must pass clean-environment chat+embedding examples, complete IA/reference/security/compliance/operations/migration coverage, generated-reference drift, link/snippet/schema, accessibility/readability, terminology, versioning and release-documentation gates. RunPod must use the committed current `rest.runpod.io/v1` OpenAPI/REST boundary with fake contracts and explicit opt-in live evidence; safe single-GPU on-demand Pod + independent-volume chat and embedding workflows must plan, launch, reconcile, checkpoint/resume, fetch, stop/terminate/cleanup and orphan-scan without secrets or duplicate resources; required QLoRA/distributed/architecture/cost/recovery gates must pass before being advertised. The release must explicitly and accurately report the four unresolved boundaries: no verified provider hard-dollar cap, no established generic Pod exec/log-stream REST, no established spot field/eviction semantics, and no established direct Secrets resource representation. Each stays fail-closed with user-side time/cost enforcement, durable events plus SSH/Jupyter/connect guidance, on-demand Pods, and verified-only environment/secret injection respectively. Serverless remains bounded evaluation/inference/serving/fleet scope and is never the MVP training scheduler; lack of live evidence yields an honest unavailable status.

A separate acceptance reviewer validates the entire implementation against this amended plan. The supervisor executes exactly one phase at a time and cannot declare the branch ready after Phase 18 or while any appended phase or global gate is incomplete. All implementation, supervision, review, documentation acceptance, live-smoke assessment and final acceptance work must use model preset `low` per the initiative override.
